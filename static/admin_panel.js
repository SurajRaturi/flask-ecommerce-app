...
btn.addEventListener("click", async () => {
  ...
  let add_response = await fetch("/Category", {
    method: "POST",
    headers: {
  ...
  if (add_response.status == 201) {
    ...
    400 <= add_response.status < 422 ||
    422 < add_response.status < 500
  ) {
    alert(data.message);
  }
...
delete_btn.addEventListener("click", async () => {
  let delete_category_response = await fetch(
    `/Catgeory/${delete_category_input.value}`,
    {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("access_token")}`,
