window.addEventListener("pageshow",(e)=>{
    ...
    let quantity_update = async () => {
        ...
        for (const button of items) {
            ...
            await fetch(
                `http://192.168.1.37:5000/updatecart/${Number(new_quantity)}/${button.id}`,
                {
                    method: "PUT",
                    headers: {
    ...
    async function get_cart_items() {
        ...
        let get_cart = await fetch("http://192.168.1.37:5000/User_Cart", {
            headers: {
                "Authorization": `Bearer ${localStorage.getItem("access_token")}`
            }
        });
    ...
    document.querySelector("#button button").addEventListener("click",async()=>{
        ...
        let order_response=await fetch("http://192.168.1.37:5000/OrderNow",{
            method:"POST",
            headers:{
                "Authorization":`Bearer ${localStorage.getItem("access_token")}`
            }
